 ...
 *  and certificate signed by a CA. This current version is highly insecure. (Did not have time to do it)
 *  
 */
@Configuration
public class OAuth2SecurityConfiguration {
	...
	protected static class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
		...
		@Autowired
		private MutiboUserRepository userRepo;
		
		@Autowired
		protected void registerAuthentication(
				final AuthenticationManagerBuilder auth) throws Exception {
			//auth.userDetailsService(userDetailsService);
			auth.userDetailsService(new CustomUserDetailService(userRepo));
		}
	...
	protected static class ResourceServer extends
		...
		public void configure(HttpSecurity http) throws Exception {
			...
			/* TODO: future impl for create users
			http
			.anonymous()
				.and()
					.authorizeRequests()
			...
			*/
			http
			.authorizeRequests()
				.antMatchers(HttpMethod.GET, "set/*").authenticated();
			
			http
			.authorizeRequests()
				.antMatchers(HttpMethod.POST, "set/*").authenticated();
			
			http
			.authorizeRequests()
				.antMatchers(HttpMethod.DELETE, "set/*").authenticated();
	...
	protected static class OAuth2Config extends
		 ...
		 * system. This is a using hard-coded lists of clients / users / passwords
		 * 
		 * @param auth
		 * @throws Exception
		 */
		public OAuth2Config() throws Exception {
			...
			combinedService_ = new ClientAndUserDetailsService(csvc, new CustomUserDetailService());
		}
	...
	//
	// TODO: Change temp keystore with a secure one
    @Bean
